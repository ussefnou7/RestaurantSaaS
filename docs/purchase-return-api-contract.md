...
```json
{ "success": false, "message": "Only DRAFT returns can be completed", "timestamp": "2026-06-27T10:15:30Z" }
```

| HTTP | When |
|---|---|
| `400 Bad Request` | Validation failure. `message` = `"<field>: <reason>"` (e.g. `"quantity: must be greater than 0"`). |
...
### Lifecycle
```
DRAFT ──complete──▶ COMPLETE ──post──▶ POSTED   (terminal)
  │                    │
...
- `post` applies stock-out to inventory at the original purchase cost; **irreversible**
  (no unpost — correct mistakes with a new document/process).
...
- Body:
```json
{ "originalLineId": 31, "quantity": 3, "notes": "optional" }
```
| field | type | required | notes |
|---|---|---|---|
| `originalLineId` | long | yes | Must belong to this return's original invoice. |
| `quantity` | decimal | yes | `> 0`; material, UOM and unit cost are derived from the original line. |
| `notes` | string | no | |
...
- **409** if not COMPLETE, or already posted. Irreversible.
